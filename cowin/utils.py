...
def collect_user_details(request_header):
    ...
    vaccine_type = vaccine_types[
        0]  # if all([beneficiary['status'] == 'Partially Vaccinated' for beneficiary in beneficiary_dtls]) else None
    if not vaccine_type:
        print(
            "\n================================= Vaccine Info =================================\n"
        )
...
def check_calendar_by_district(request_header, vaccine_type, location_dtls,
                               start_date, minimum_slots, min_age_booking,
                               fee_type):
    """
    This function
    ...
    try:
        ...
        for location in location_dtls:
            resp = requests.get(base_url.format(location['district_id'],
                                                start_date),
            ...
            if resp.status_code == 401:
                ...
                if 'centers' in resp:
                    print(
                        f"Centers available in {location['district_name']} from {start_date} : {len(resp['centers'])}"
                    )
...
def check_calendar_by_pincode(request_header, vaccine_type, location_dtls,
                              start_date, minimum_slots, min_age_booking,
                              fee_type):
    """
    This function
...
def check_and_book(request_header, beneficiary_dtls, location_dtls,
                   search_option, **kwargs):
    """
    This function
    ...
    try:
        ...
        options = sorted(options,
                         key=lambda k:
                         (k['district'].lower(), k['pincode'], k['name'].lower(
        ...
        if choice == '.':
            ...
            try:
                ...
                new_req = {
                    ...
                    'center_id':
                        options[choice[0] - 1]['center_id'],
                    'session_id':
                        options[choice[0] - 1]['session_id'],
                    'slot':
                        options[choice[0] - 1]['slots'][choice[1] - 1]
                }
...
def get_vaccine_preference():
    ...
    preference = int(preference) if preference and int(preference) in [0, 1, 2
                                                                      ] else 0
