...
class MySql:
    ...
    def connect(self):
        ...
        for i in range(RETRY_COUNT):
            try:
                ...
                if i == RETRY_COUNT - 1:
                    # Since this was last retry.. Raise the exception.
                    print("MySQL connection failed - {0}".format(str(exc)))
                    raise
                print("MySQL connection failed due to {0}.. Retrying..".format(
                    str(exc)))
                time.sleep(randint(1, 10))
            except Exception as exc:
                print("MySQL connect failed - {0}".format(str(exc)))
                raise
            else:
                return True
    ...
    def getAll(self, table=None, fields='*', where=None, order=None,
               limit=None):
        """Get all results
        ...
        self.__last_cur = cur = self._select(table, fields, where, order, limit)
        result = cur.fetchall()
    ...
    def load_dict(self, rows):
        data = []
        columns = tuple(
            [d[0].decode('utf8') for d in self.__last_cur.description])
        for row in self.__last_cur:
    ...
    def leftJoin(self,
        ...
        cur = self._select_join(tables, fields, join_fields, where, order,
                                limit)
        result = cur.fetchall()
    ...
    def insertBatch(self, table, data):
        ...
        sql = "INSERT INTO %s (%s) VALUES %s" % (table, query[0], query[1])
        flattened_values = [
            v for sublist in data for k, v in sublist.iteritems()
        ]
    ...
    def update(self, table, data, where=None):
        ...
        return self.query(
            sql,
            data.values() +
            where[1] if where and len(where) > 1 else data.values()).rowcount
    ...
    def query(self, sql, params=None):
        ...
        for i in range(RETRY_COUNT):
            ...
            try:
                ...
                # Some Mysql Error, which may be acquiring lock, server timeout etc
                # Retry in such cases
                if i == RETRY_COUNT - 1:
                    # Since this was last retry.. Raise the exception.
                    print("MySQL Query failed - {0}".format(str(e)))
                    raise
                print("MySQL Query failed due to {0}.. Retrying..".format(
                    str(e)))
                # Since connection might have failed. try reconnecting..
                try:
                    """In case of "Commands out of sync" error... Just to be on
					safer side, we are disconnecting the connection
					For all other errors, we are creating a fresh connection"""
                    if e[0] == 2014:
                        self.end()
                    self.connect()
                except:
                ...
                print("MySQL Query failed - {0}".format(str(exc)))
                raise

    def run_query(self, sql):
        self.__last_cur = cur = self.query(sql)
    ...
    def _serialize_batch_insert(self, data):
        ...
        v = "(%s)" % ",".join(
            tuple("%s".rstrip(',') for v in range(len(data[0]))))
